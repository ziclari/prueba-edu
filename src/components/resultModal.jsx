...
export default function ResultModal({ type = "neutral",title, message, score, onClose, isOpen }) {
  ...
  return (
    <div className="fixed bottom-24 right-16 z-50 w-xl">
      <div
        className={`
          flex items-center gap-3 px-4 py-3 rounded-xl border bg-white border-gray-200 shadow-md
      ...
      >
        ...
        <div className="flex-1 space-y-2 mt-8 mb-8">
            <p className="font-bold font-primary text-xl">{title}</p>
            <p className="text-lg font-primary">{message}</p>
            <p className="text-lg font-primary text-gray-700">{score}</p>
        </div>
        <button
          onClick={onClose}
          className="text-gray-500 hover:text-gray-700 absolute top-4 right-4 cursor-pointer"
        >
